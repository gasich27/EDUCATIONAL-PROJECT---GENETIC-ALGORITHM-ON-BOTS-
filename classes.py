...
class Bot:
    def __init__(self, genes, mutant=False, fixed=False):
        ...
        for i in range(MIND_SIZE):
            self.mem[i] = genes[i] if i < len(genes) else 0
    ...
    def step(self):
        ...
        if self.damage(1) <= 0 and self.world:
    ...
    def reset(self):
        self.mem[ADR] = 0
        self.mem[HP] = 35
        self.mem[C_BLUE] = 200 if not self.mutant else 255
        self.mem[C_GREEN] = 0
...
class GA:
    ...
    def init(self):
        ...
        for _ in range(self.size):
            genes = [random.randint(0, 63) for _ in range(MIND_SIZE)]
            self.bots.append(Bot(genes))
    ...
    def reproduce(self, parents):
        ...
        for parent in parents:
            ...
            for _ in range(random.randint(1, 5)):
                idx = random.randint(0, MIND_SIZE - 1)
                genes[idx] = random.randint(0, 63)
...
class Sim:
    ...
    def draw(self, screen, cell_size, gen, tick):
        screen.fill((0, 0, 0))
        w = self.world
        ...
        pygame.draw.rect(
            screen,
            (255, 255, 255),
            (0, 0, w.w * cell_size, panel_height)
        )

        font = pygame.font.SysFont("steppe", 28, bold=False)
        alive = len([b for b in self.ga.bots if b.alive()])
        text = f"Gen: {gen} | Tick: {tick} | Alive: {alive}/{len(self.ga.bots)}"
        ...
        for y in range(w.h):
            for x in range(w.w):
                ...
                rect = pygame.Rect(x * cell_size, offset_y + y * cell_size, cell_size, cell_size)

                pygame.draw.rect(screen, color, rect)
                pygame.draw.rect(screen, (54, 54, 54), rect, 1)

        for b in w.bots:
            if not b.alive():
                continue
            x, y = b.mem[X_POS], b.mem[Y_POS]
    ...
    def run_gen(self, screen, clock, cell_size, gen):
        tick = 0
        speed = 2000
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        speed = 10 if speed == 2000 else 2000

            self.world.update()
            self.draw(screen, cell_size, gen, tick)
            pygame.display.flip()
            clock.tick(speed)
            tick += 1
    ...
    def run(self, gens, screen, clock, cell_size):
        for gen in range(gens):
            self.init_world()
            self.load_bots()
            
            ticks, cont = self.run_gen(screen, clock, cell_size, gen)
            if not cont:
                break
...
def main():
    ...
    w, h = 50, 30
    cell = 25
    screen = pygame.display.set_mode((w * cell, h * cell + 45))
    pygame.display.set_caption("BIOS-life")
    ...
    try:
        sim.run(5000, screen, clock, cell)
